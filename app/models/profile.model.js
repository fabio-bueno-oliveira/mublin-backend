...
Profile.projects = (username, result) => {
  sql.query(`SELECT up.confirmed, up.joined_in, up.left_in, up.show_on_profile, up.portfolio, up.created, projects.id, projects.name, projects.username, CONCAT('https://ik.imagekit.io/mublin/projects/tr:h-200,w-200,c-maintain_ratio/',projects.picture) AS picture, up.featured, projects_types.name_ptbr AS type, users_projects_relationship.title_ptbr AS workTitle, users_projects_relationship.icon AS workIcon, r1.description_ptbr AS role1, r2.description_ptbr AS role2, r3.description_ptbr AS role3 FROM users_projects AS up LEFT JOIN projects ON up.id_project_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN users_projects_relationship ON up.status = users_projects_relationship.id LEFT JOIN roles AS r1 ON up.main_role_fk = r1.id LEFT JOIN roles AS r2 ON up.second_role_fk = r2.id LEFT JOIN roles AS r3 ON up.third_role_fk = r3.id WHERE up.id_user_fk = (SELECT users.id FROM users WHERE users.username = '${username}') AND up.confirmed IN(0,1,2) AND projects.public = 1 ORDER BY up.featured DESC, up.status ASC, up.confirmed ASC`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Profile.followers = (username, result) => {
  sql.query(`SELECT uf.id, uf.id_follower AS followerId, uf.id_followed AS followedId, u.id, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.id,'/',u.picture) AS picture, u.verified FROM users_followers AS uf LEFT JOIN users AS u ON uf.id_follower = u.id WHERE uf.id_followed = (SELECT users.id FROM users WHERE users.username = '${username}') AND u.status = 1`, 
  (err, res) => {
    if (err) {
      //console.log("error: ", err);
      result(null, err);
...
Profile.gear = (username, result) => {
  sql.query(`
  SELECT ug.featured, ug.for_sale AS forSale, ug.price, ug.currently_using AS currentlyUsing, ug.id_product AS productId, it.name_ptbr AS tuning, it.description AS tuningDescription, brands.name AS brandName, CONCAT('https://ik.imagekit.io/mublin/products/brands/tr:h-200,w-200,cm-pad_resize,bg-FFFFFF/',brands.logo) AS brandLogo, bp.name AS productName, CONCAT('https://ik.imagekit.io/mublin/products/tr:h-200,w-200,cm-pad_resize,bg-FFFFFF/',bp.picture) AS picture, bp.id_brand AS brandId, bc.name_ptbr AS category FROM users_gear AS ug LEFT JOIN brands_products AS bp ON ug.id_product = bp.id LEFT JOIN brands ON bp.id_brand = brands.id LEFT JOIN brands_categories AS bc ON bp.id_category = bc.id LEFT JOIN instrument_tunings AS it ON ug.tuning = it.id WHERE ug.id_user = (SELECT users.id FROM users WHERE users.username = '${username}') ORDER BY ug.featured DESC, ug.currently_using DESC, ug.created DESC; 
  
  SELECT bc.name_ptbr AS category, bc.macro_category, COUNT(ug.id_product) as total FROM users_gear AS ug LEFT JOIN brands_products ON ug.id_product = brands_products.id LEFT JOIN brands_categories AS bc ON brands_products.id_category = bc.id WHERE ug.id_user = (SELECT users.id FROM users WHERE users.username = '${username}') GROUP BY brands_products.id_category ORDER BY ug.featured DESC, ug.currently_using DESC, ug.created DESC
  `,  (err, results) => {
    if (err) {
      //console.log("error: ", err);
      result(err, null);
