...
Profile.infos = (username, result) => {
  sql.query(`SELECT u.id, u.name, u.lastname, u.username, u.gender, u.bio, u.email, u.phone_mobile AS phone, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.picture) AS picture, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-580,w-580,c-maintain_ratio/',u.picture) AS pictureLarge, u.picture_cover, u.open_to_work AS openToWork, u.open_to_work_text AS openToWorkText, u.first_access AS firstAccess, availability_statuses.id AS availabilityId, availability_statuses.title_ptbr AS availabilityTitle, availability_statuses.color AS availabilityColor, availability_focus AS availabilityFocusId, avf.title_ptbr AS availabilityFocus, c.name AS country, r.name AS region, r.uf AS regionUF, ci.name AS city, IF(u.payment_plan=1,'Free', 'Pro') AS plan, u.legend_badge AS legend, u.verified, u.instagram, u.tiktok, u.website, u.public FROM users AS u LEFT JOIN availability_statuses ON u.availability_status = availability_statuses.id LEFT JOIN availability_focuses AS avf ON u.availability_focus = avf.id LEFT JOIN countries AS c ON u.id_country_fk = c.id LEFT JOIN regions AS r ON u.id_region_fk = r.id LEFT JOIN cities AS ci ON u.id_city_fk = ci.id WHERE u.username = '${username}' AND u.status = 1 LIMIT 1`, (err, res) => {
    if (err) {
...
Profile.followers = (username, result) => {
  sql.query(`SELECT uf.id, uf.id_follower AS followerId, uf.id_followed AS followedId, u.id AS userId, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.picture) AS picture, u.verified, u.legend_badge, u.open_to_work AS openToWork, u.open_to_work_text AS openToWorkText FROM users_followers AS uf LEFT JOIN users AS u ON uf.id_follower = u.id WHERE uf.id_followed = (SELECT users.id FROM users WHERE users.username = '${username}') AND u.status = 1 ORDER BY uf.id DESC`, 
  (err, res) => {
    if (err) {
      //console.log("error: ", err);
      result(null, err);
...
Profile.following = (username, result) => {
  sql.query(`SELECT users_followers.id, users_followers.id_follower AS followerId, users_followers.id_followed AS followedId, u.id AS userId, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.picture) AS picture, u.verified, u.legend_badge, u.open_to_work AS openToWork, u.open_to_work_text AS openToWorkText FROM users_followers LEFT JOIN users AS u ON users_followers.id_followed = u.id WHERE users_followers.id_follower = (SELECT users.id FROM users WHERE users.username = '${username}') AND u.status = 1 ORDER BY users_followers.id DESC`, 
  (err, res) => {
    if (err) {
      //console.log("error: ", err);
      result(null, err);
...
Profile.inspired = (username, result) => {
  sql.query(`SELECT uf.id, uf.id_follower AS followerId, uf.id_followed AS followedId, u.id AS userId, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.picture) AS picture, u.verified, u.legend_badge FROM users_followers AS uf LEFT JOIN users AS u ON uf.id_follower = u.id WHERE uf.id_followed = (SELECT users.id FROM users WHERE users.username = '${username}') AND u.status = 1 AND uf.inspiration = 1 ORDER BY uf.id DESC`, 
  (err, res) => {
    if (err) {
      //console.log("error: ", err);
      result(null, err);
...
Profile.gearSetup = (username, itemId, result) => {
  sql.query(`SELECT setup.id, setup.name, setup.image, setup.description, UNIX_TIMESTAMP(setup.created) AS created FROM users_gear_setup AS setup WHERE setup.id_user = (SELECT users.id FROM users WHERE users.username = '${username}') AND setup.id = ${itemId} LIMIT 1`, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Profile.testimonials = (username, result) => {
  sql.query(`SELECT users_testimonials.id, DATE_FORMAT(users_testimonials.created,'%d/%m/%Y') AS created, users_testimonials.id_user_from_fk AS friendId, users_testimonials.title, users_testimonials.testimonial, CONCAT(users.name,' ',users.lastname) AS friendName, users.username AS friendUsername, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',users.picture) AS friendPicture, IF(users.payment_plan=1,'Free', 'Pro') AS friendPlan FROM users_testimonials LEFT JOIN users ON users_testimonials.id_user_from_fk = users.id WHERE users_testimonials.id_user_to_fk = (SELECT users.id FROM users WHERE users.username = '${username}') AND users.status = 1 ORDER BY users_testimonials.created DESC`, (err, res) => {
    if (err) {
      //console.log("error: ", err);
      result(err, null);
      return;
