...
Notification.feed = (loggedID, result) => {
  let x = jwt.verify(loggedID.slice(7), process.env.JWT_SECRET)
  sql.query(`SELECT f.id, f.id_item_fk AS relatedItemId, f.extra_text AS extraText, f.extra_info AS extraInfo, UNIX_TIMESTAMP(f.created) AS created, u.name AS relatedUserName, u.lastname AS relatedUserLastname, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.id,'/',u.picture) AS relatedUserPicture, u.username AS relatedUserUsername, r.description_ptbr AS relatedUserMainRole, c.name AS relatedUserCity, rg.name AS relatedUserRegion, IF(u.payment_plan=1,'Free', 'Pro') AS relatedUserPlan, IF(feed_types.category='project',projects.name, '') AS relatedProjectName, IF(feed_types.category='project',projects.username, '') AS relatedProjectUsername, IF(feed_types.category='project',CONCAT('https://ik.imagekit.io/mublin/projects/tr:h-200,w-200,c-maintain_ratio/',projects.id,'/',projects.picture), '') AS relatedProjectPicture, IF(feed_types.category='project',projects_types.name_ptbr, '') AS relatedProjectType, feed_types.text_ptbr AS action, feed_types.category, feed_types.show_only_as_notification, feed_types.id AS categoryId, IF(feed_types.category='event',events.id, '') AS relatedEventId, IF(feed_types.category='event',events.title, '') AS relatedEventTitle, COUNT(feed_likes.id) AS likes, (SELECT COUNT(feed_likes.id) FROM feed_likes WHERE feed_likes.id_feed_item = f.id AND id_user = ${x.result.id}) AS likedByMe FROM feed AS f LEFT JOIN users AS u ON f.id_user_1_fk = u.id LEFT JOIN users_roles AS ur ON u.id = ur.id_user_fk LEFT JOIN roles AS r ON ur.id_role_fk = r.id LEFT JOIN cities AS c ON u.id_city_fk = c.id LEFT JOIN regions AS rg ON u.id_region_fk = rg.id LEFT JOIN projects ON f.id_item_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN feed_types ON f.id_feed_type_fk = feed_types.id LEFT JOIN events ON f.id_item_fk = events.id LEFT JOIN feed_likes ON f.id = feed_likes.id_feed_item WHERE f.id_item_fk IN (SELECT project_fans.id_fan_fk FROM project_fans WHERE project_fans.id_fan_fk = ${x.result.id}) OR f.id_item_fk IN (SELECT users_projects.id_project_fk FROM users_projects WHERE users_projects.id_user_fk = ${x.result.id}) OR f.id_user_1_fk IN (SELECT users_followers.id_followed FROM users_followers WHERE users_followers.id_follower = ${x.result.id}) OR f.id_user_1_fk = ${x.result.id} AND u.id IS NOT NULL AND u.status = ${x.result.id} GROUP BY f.id HAVING feed_types.show_only_as_notification = 0 ORDER BY f.created DESC, ur.main_activity DESC LIMIT 50`, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
...
Notification.simpleFeed = (loggedID, result) => {
  let x = jwt.verify(loggedID.slice(7), process.env.JWT_SECRET)
  sql.query(`SELECT feed.id, feed.id_item_fk AS relatedItemId, feed.extra_text AS extraText, feed.image AS image, feed.extra_info AS extraInfo, UNIX_TIMESTAMP(feed.created) AS created, users.name AS relatedUserName, users.lastname AS relatedUserLastname, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',users.id,'/',users.picture) AS relatedUserPicture, users.username AS relatedUserUsername, IF(users.payment_plan=1,'Free', 'Pro') AS relatedUserPlan, users.verified AS relatedUserVerified, feed_types.text_ptbr AS action, feed_types.category, feed_types.show_only_as_notification, feed_types.id AS categoryId, COUNT(feed_likes.id) AS likes, (SELECT COUNT(feed_likes.id) FROM feed_likes WHERE feed_likes.id_feed_item = feed.id AND id_user = ${x.result.id}) AS likedByMe FROM feed LEFT JOIN users ON feed.id_user_1_fk = users.id LEFT JOIN feed_types ON feed.id_feed_type_fk = feed_types.id LEFT JOIN feed_likes ON feed.id = feed_likes.id_feed_item WHERE feed.id_item_fk IN (SELECT project_fans.id_fan_fk FROM project_fans WHERE project_fans.id_fan_fk = ${x.result.id}) OR feed.id_item_fk IN (SELECT users_projects.id_project_fk FROM users_projects WHERE users_projects.id_user_fk = ${x.result.id}) OR feed.id_user_1_fk IN (SELECT users_followers.id_followed FROM users_followers WHERE users_followers.id_follower = ${x.result.id}) OR feed.id_user_1_fk = ${x.result.id} AND users.id IS NOT NULL AND users.status = 1 GROUP BY feed.id HAVING feed_types.id = 8 AND feed_types.show_only_as_notification = 0 ORDER BY feed.created DESC LIMIT 100`, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
