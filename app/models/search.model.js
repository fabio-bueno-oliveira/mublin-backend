...
Search.findUsersByKeyword = (keyword, userCity, result) => {
  sql.query(`SELECT u.id, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.id,'/',u.picture) AS picture, u.public AS publicProfile, IF(u.payment_plan=1,'Free', 'Pro') AS plan, u.status, u.legend_badge AS legend, u.verified, cities.name AS city, regions.name AS region, countries.name AS country, roles.name_ptbr AS roleName, roles.description_ptbr AS mainRole, roles.instrumentalist, projects.name AS projectRelated, projects.public AS projectPublic, availability_statuses.title_ptbr AS availabilityStatus, availability_statuses.color AS availability_color, projects_types.name_ptbr AS projectType, (SELECT COUNT(projects.id) FROM users_projects JOIN projects ON users_projects.id_project_fk = projects.id WHERE users_projects.id_user_fk = u.id AND users_projects.show_on_profile = 1) AS totalProjects FROM users AS u LEFT JOIN users_roles ON u.id = users_roles.id_user_fk LEFT OUTER JOIN roles ON users_roles.id_role_fk = roles.id LEFT OUTER JOIN users_projects ON u.id = users_projects.id_user_fk LEFT OUTER JOIN projects ON users_projects.id_project_fk = projects.id LEFT OUTER JOIN projects_types ON projects.type = projects_types.id LEFT JOIN availability_statuses ON u.availability_status = availability_statuses.id LEFT JOIN cities ON u.id_city_fk = cities.id LEFT JOIN regions ON u.id_region_fk = regions.id LEFT JOIN countries ON u.id_country_fk = countries.id WHERE MATCH(u.name, u.lastname) AGAINST ('%${keyword}%') OR u.name LIKE '%${keyword}%' OR u.lastname LIKE '%${keyword}%' OR cities.name LIKE '%${keyword}%' OR roles.name_ptbr LIKE '%${keyword}%' OR roles.description_ptbr LIKE '%${keyword}%' OR projects.name LIKE '%${keyword}%' GROUP BY u.id HAVING u.public = 1 AND u.status = 1 ORDER BY u.name ASC, u.lastname ASC, (cities.name = '%${userCity}%') DESC LIMIT 100`, (err, res) => {
    if (err) {
