...
Search.findUsersProjectsFromUserSearch = (userId, result) => {
  sql.query(`SELECT up.id_project_fk AS projectId, p.name, p.username, CONCAT('https://ik.imagekit.io/mublin/projects/tr:h-200,w-200,c-maintain_ratio/',p.id,'/',p.picture) AS picture, p.public FROM users_projects AS up LEFT JOIN projects AS p ON up.id_project_fk = p.id WHERE up.id_user_fk = ${userId} AND p.public = 1 ORDER BY up.featured DESC, up.joined_in DESC, up.portfolio DESC, p.name ASC LIMIT 100`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
