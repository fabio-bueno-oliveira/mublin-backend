...
Search.findGearByKeyword = (keyword, result) => {
  sql.query(`SELECT p.id AS productId, p.name AS productName, p.picture AS productPicture, b.name AS brand, b.slug AS brandSlug, b.logo AS brandLogo, c.name_ptbr AS colorPTBR, c.name AS color, pc.name_ptbr, pc.macro_category, (SELECT COUNT(id) FROM users_gear WHERE id_product = p.id) AS totalOwners FROM products AS p LEFT JOIN brands AS b ON p.id_brand = b.id LEFT JOIN colors AS c ON p.color = c.id LEFT JOIN products_categories AS pc ON p.id_category = pc.id LEFT JOIN products_colors ON p.id = products_colors.id_product LEFT JOIN colors AS otherColors ON products_colors.id_color = otherColors.id WHERE p.name LIKE '%${keyword}%' OR b.name LIKE '%${keyword}%' OR c.name LIKE '%${keyword}%' OR c.name_ptbr LIKE '%${keyword}%' OR pc.name_ptbr LIKE '%${keyword}%' OR otherColors.name LIKE '%${keyword}%' OR otherColors.name_ptbr LIKE '%${keyword}%' OR CONCAT(b.name,' ',p.name) LIKE '%${keyword}%' OR (MATCH (p.name) AGAINST ('%${keyword}%') AND MATCH (b.name) AGAINST ('%${keyword}%')) OR (MATCH (p.name) AGAINST ('%${keyword}%') AND MATCH (c.name) AGAINST ('%${keyword}%')) OR (MATCH (p.name) AGAINST ('%${keyword}%') AND MATCH (c.name_ptbr) AGAINST ('%${keyword}%')) OR (MATCH (b.name) AGAINST ('%${keyword}%') AND MATCH (c.name) AGAINST ('%${keyword}%')) OR (MATCH (b.name) AGAINST ('%${keyword}%') AND MATCH (c.name_ptbr) AGAINST ('%${keyword}%')) OR (MATCH (b.name) AGAINST ('%${keyword}%') AND MATCH (pc.name_ptbr) AGAINST ('%${keyword}%')) OR (MATCH (c.name_ptbr) AGAINST ('%${keyword}%') AND MATCH (pc.name_ptbr) AGAINST ('%${keyword}%')) ORDER BY p.name ASC, b.name ASC LIMIT 50`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Search.getRandomFeaturedUsers = (loggedUserId, result) => {
  let x = jwt.verify(loggedUserId.slice(7), process.env.JWT_SECRET)
  sql.query(`SELECT u.id, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.id,'/',u.picture) AS picture, u.verified, u.legend_badge AS legend, u.open_to_work AS openToWork, u.open_to_work_text AS openToWorkText, c.name AS city, r.name AS region, r.uf, rl.description_ptbr AS role FROM users AS u LEFT JOIN cities AS c ON u.id_city_fk = c.id LEFT JOIN regions AS r ON u.id_region_fk = r.id LEFT JOIN users_roles AS ur ON u.id = ur.id_user_fk LEFT JOIN roles AS rl ON ur.id_role_fk = rl.id WHERE u.status = 1 AND u.public = 1 GROUP BY u.username ORDER BY ur.main_activity DESC, u.legend_badge DESC, u.verified DESC, RAND() LIMIT 3;`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
