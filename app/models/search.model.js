...
Search.findProjectsByKeyword = (keyword, userCity, result) => {
  sql.query(`SELECT projects.id, projects.name, projects.username, CONCAT('https://ik.imagekit.io/mublin/projects/tr:h-200,w-200,c-maintain_ratio/',projects.id,'/',projects.picture) AS picture, projects.public, cities.name AS city, regions.name AS region, countries.name AS country, genre1.name AS mainGenre, genre2.name AS secondGenre, genre3.name AS thirdGenre FROM projects LEFT JOIN genres AS genre1 ON projects.id_genre_1_fk = genre1.id LEFT JOIN genres AS genre2 ON projects.id_genre_2_fk = genre2.id LEFT JOIN genres AS genre3 ON projects.id_genre_3_fk = genre3.id LEFT JOIN cities ON projects.id_city_fk = cities.id LEFT JOIN regions ON projects.id_region_fk = regions.id LEFT JOIN countries ON projects.id_country_fk = countries.id WHERE projects.name LIKE '%${keyword}%' OR projects.username LIKE '%${keyword}%' HAVING projects.public = 1 ORDER BY projects.name ASC, (cities.name = '%${userCity}%') DESC`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
