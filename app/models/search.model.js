...
Search.findUsersByKeyword = (keyword, userCity, result) => {
  sql.query(`SELECT users.id, users.name, users.lastname, users.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',users.id,'/',users.picture) AS picture, users.public AS publicProfile, IF(users.payment_plan=1,'Free', 'Pro') AS plan, users.status, cities.name AS city, regions.name AS region, countries.name AS country, roles.name_ptbr AS roleName, roles.description_ptbr AS mainRole, projects.name AS projectRelated, availability_statuses.title_ptbr AS availabilityStatus, availability_statuses.color AS availability_color FROM users LEFT JOIN users_roles ON users.id = users_roles.id_user_fk LEFT OUTER JOIN roles ON users_roles.id_role_fk = roles.id LEFT OUTER JOIN users_projects ON users.id = users_projects.id_user_fk LEFT OUTER JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN availability_statuses ON users.availability_status = availability_statuses.id LEFT JOIN cities ON users.id_city_fk = cities.id LEFT JOIN regions ON users.id_region_fk = regions.id LEFT JOIN countries ON users.id_country_fk = countries.id WHERE MATCH(users.name, users.lastname) AGAINST ('%${keyword}%') OR users.name LIKE '%${keyword}%' OR users.lastname LIKE '%${keyword}%' OR cities.name LIKE '%${keyword}%' OR roles.name_ptbr LIKE '%${keyword}%' OR roles.description_ptbr LIKE '%${keyword}%' OR projects.name LIKE '%${keyword}%' GROUP BY users.id HAVING users.public = 1 AND users.status = 1 ORDER BY users.name ASC, (cities.name = '%${userCity}%') DESC`, (err, res) => {
    if (err) {
