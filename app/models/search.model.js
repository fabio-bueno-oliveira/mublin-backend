...
Search.getRandomFeaturedUsers = (loggedUserId, result) => {
  let x = jwt.verify(loggedUserId.slice(7), process.env.JWT_SECRET)
  sql.query(`SELECT u.id, u.name, u.lastname, u.username, CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',u.id,'/',u.picture) AS picture, u.verified, u.legend_badge, c.name AS city, r.name AS region, r.uf FROM users AS u LEFT JOIN cities AS c ON u.id_city_fk = c.id LEFT JOIN regions AS r ON u.id_region_fk = r.id WHERE u.status = 1 AND u.public = 1 ORDER BY u.legend_badge DESC, u.verified, RAND() DESC LIMIT 3`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
