...
Project.findAllByUser = (userId, type, result) => {
  ...
  LEFT JOIN genres AS g1 ON projects.id_genre_1_fk = g1.id 
  LEFT JOIN genres AS g2 ON projects.id_genre_2_fk = g2.id 
  LEFT JOIN genres AS g3 ON projects.id_genre_3_fk = g3.id 
  ...
  LEFT JOIN projects_status ON projects.activity_status = projects_status.id 

  WHERE usrsprjcts.id_user_fk = ${userId} AND usrsprjcts.confirmed IN(0,1,2) AND projects.id IS NOT NULL AND usrsprjcts.portfolio IN(${filterType(type)})
  
  GROUP BY usrsprjcts.id 
  
  ORDER BY projects.end_year IS NOT NULL ASC, usrsprjcts.left_in IS NOT NULL ASC, usrsprjcts.confirmed DESC, usrsprjcts.featured DESC, usrsprjcts.joined_in DESC, usrsprjcts.status ASC, projects.name ASC, events.date_opening DESC;

  SELECT usrsprjcts.confirmed, usrsprjcts.joined_in, usrsprjcts.left_in, usrsprjcts.portfolio, projects.id AS projectid, projects.name, projects.username FROM users_projects AS usrsprjcts LEFT JOIN projects ON usrsprjcts.id_project_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN users_projects_status ON usrsprjcts.status = users_projects_status.id WHERE usrsprjcts.id_user_fk = ${userId} AND usrsprjcts.confirmed IN(0,1,2) AND projects.id IS NOT NULL GROUP BY usrsprjcts.id ORDER BY projects.end_year IS NOT NULL ASC, usrsprjcts.left_in IS NOT NULL ASC, usrsprjcts.confirmed DESC, usrsprjcts.featured DESC, usrsprjcts.joined_in DESC, usrsprjcts.status ASC, projects.name ASC;
