...
Project.findAllByUser = (userId, type, result) => {
  ...
  SELECT users.id AS userId, users.name AS userName, users.lastname AS userLastname, users.username AS userUsername, users.picture AS userPicture, projects.id AS projectId, projects.username AS projectUsername FROM users_projects LEFT JOIN users ON users_projects.id_user_fk = users.id LEFT JOIN projects ON users_projects.id_project_fk = projects.id WHERE users_projects.confirmed = ${userId} AND users_projects.id_project_fk IN (SELECT projects.id FROM users_projects LEFT JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN users_projects_status ON users_projects.status = users_projects_status.id WHERE users_projects.id_user_fk = ${userId} AND users_projects.confirmed IN(0,1,2) AND projects.id IS NOT NULL GROUP BY users_projects.id) ORDER BY projects.name;
