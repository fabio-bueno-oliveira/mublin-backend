...
Project.findAllByUser = (userId, result) => {
  ...
  LEFT JOIN (SELECT id_project, DATE_FORMAT(goal_date,'%d/%m/%Y') AS goal_date, goal_description FROM projects_goals WHERE goal_date >= CURDATE() ORDER BY goal_date DESC) AS projects_goals ON projects.id = projects_goals.id_project 

  LEFT JOIN (SELECT id_project, id_user, DATE_FORMAT(goal_date,'%d/%m/%Y') AS goal_date, goal_description FROM projects_goals WHERE goal_date >= CURDATE() AND id_user = ${userId} ORDER BY goal_date DESC) AS projects_goals_user ON projects.id = projects_goals_user.id_project 
