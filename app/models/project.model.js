...
Project.findAllByUser = (userId, type, result) => {
  ...
  LEFT JOIN (SELECT id_project, DATE_FORMAT(created,'%d/%m/%Y') AS note_date, note FROM project_notes ORDER BY created DESC LIMIT 1) AS project_notes ON projects.id = project_notes.id_project 
