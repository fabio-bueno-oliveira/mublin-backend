...
Project.findAllByUser = (userId, type, result) => {
  ...
  sql.query(`SELECT users_projects.id, users_projects.confirmed, users_projects.status, users_projects.active, users_projects.admin, users_projects.featured, users_projects.joined_in, users_projects.left_in AS yearLeftTheProject, users_projects.show_on_profile AS showOnProfile, users_projects.touring, users_projects.main_role_fk, users_projects.portfolio, users_projects.created, projects.id AS projectid, projects.name, projects.username, projects.type, projects.picture, projects.foundation_year AS yearFoundation, projects.end_year AS yearEnd, projects.label_show AS labelShow, projects.label_color AS labelColor, projects.label_text AS labelText, projects_types.id AS ptid, projects_types.name_ptbr AS ptname, projects_types.icon AS pticon, users_projects_status.title_ptbr AS workTitle, users_projects_status.icon AS workIcon, r1.description_ptbr AS role1, r1.icon AS role1icon, r2.description_ptbr AS role2, r2.icon AS role2icon, r3.description_ptbr AS role3, r3.icon AS role3icon, g1.name AS genre1, g2.name AS genre2, g3.name AS genre3, events.id AS nextEventId, events.title AS nextEventTitle, events.date_opening AS nextEventDateOpening, events.hour_opening AS nextEventHourOpening, events_invitations.id AS nextEventInvitationId, events_invitations.response AS nextEventInvitationResponse, DATE_FORMAT(events_invitations.created,'%d/%m/%Y às %H:%i:%s') AS nextEventInvitationDate, events_invitations.name AS nextEventInvitationNameWhoInvited, events_invitations.username AS nextEventInvitationUsernameWhoInvited, events_invitations.picture AS nextEventInvitationPictureWhoInvited, events_invitations.id_user_who_invited_fk AS nextEventInvitationUserIdWhoInvited, projects_goals.goal_date AS nextGoalDate, projects_goals.goal_description AS nextGoalDescription, projects_goals.completed AS nextGoalCompleted, projects_goals_user.goal_date AS nextUserGoalDate, projects_goals_user.goal_description AS nextUserGoalDescription, projects_goals_user.completed AS nextUserGoalCompleted, cities.name AS cityName, regions.name AS regionName, regions.uf AS regionUf, project_notes.note AS leaderLastNote, project_notes.note_date AS leaderLastNoteDate, project_notes.authorId AS leaderLastNoteAuthorId, project_notes.username AS leaderLastNoteUsername, project_notes.name AS leaderLastNoteName, project_notes.picture AS leaderLastNotePicture 

  FROM users_projects LEFT JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN cities ON projects.id_city_fk = cities.id LEFT JOIN regions ON projects.id_region_fk = regions.id LEFT JOIN users_projects_status ON users_projects.status = users_projects_status.id LEFT JOIN roles AS r1 ON users_projects.main_role_fk = r1.id LEFT JOIN roles AS r2 ON users_projects.second_role_fk = r2.id LEFT JOIN roles AS r3 ON users_projects.third_role_fk = r3.id LEFT JOIN genres AS g1 ON projects.id_genre_1_fk = g1.id LEFT JOIN genres AS g2 ON projects.id_genre_2_fk = g2.id LEFT JOIN genres AS g3 ON projects.id_genre_3_fk = g3.id 

  LEFT JOIN (SELECT id_project_fk, id, title, DATE_FORMAT(date_opening,'%d/%m/%Y') AS date_opening, hour_opening FROM events WHERE date_opening >= CURDATE() ORDER BY date_opening ASC) AS events ON projects.id = events.id_project_fk 

  LEFT JOIN (SELECT events_invitations.id, events_invitations.response, events_invitations.id_event_fk, events_invitations.created, events_invitations.id_user_who_invited_fk, users.name, users.picture, users.username FROM events_invitations LEFT JOIN users ON events_invitations.id_user_who_invited_fk = users.id) AS events_invitations ON events.id = events_invitations.id_event_fk 

  LEFT JOIN (SELECT id_project, DATE_FORMAT(goal_date,'%d/%m/%Y') AS goal_date, goal_description, completed FROM projects_goals WHERE goal_date >= CURDATE() ORDER BY goal_date DESC) AS projects_goals ON projects.id = projects_goals.id_project 
  ...
  LEFT JOIN (SELECT id_project, DATE_FORMAT(project_notes.created,'%d/%m/%Y') AS note_date, note, users.id AS authorId, users.name, users.picture, users.username FROM project_notes LEFT JOIN users ON project_notes.id_author = users.id ORDER BY project_notes.created DESC LIMIT 1) AS project_notes ON projects.id = project_notes.id_project 

  WHERE users_projects.id_user_fk = ${userId} AND users_projects.confirmed IN(0,1,2) AND projects.id IS NOT NULL AND users_projects.portfolio IN(${filterType(type)})
  ...
  SELECT users_projects.confirmed, users_projects.joined_in, users_projects.left_in, users_projects.portfolio, projects.id AS projectid, projects.name, projects.username FROM users_projects LEFT JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN users_projects_status ON users_projects.status = users_projects_status.id WHERE users_projects.id_user_fk = ${userId} AND users_projects.confirmed IN(0,1,2) AND projects.id IS NOT NULL GROUP BY users_projects.id ORDER BY projects.end_year IS NOT NULL ASC, users_projects.left_in IS NOT NULL ASC, users_projects.confirmed DESC, users_projects.featured DESC, users_projects.joined_in DESC, users_projects.status ASC, projects.name ASC;
  
  SELECT users.id AS userId, users.name AS userName, users.lastname AS userLastname, users.username AS userUsername, users.picture AS userPicture, projects.id AS projectId, projects.username AS projectUsername, users_projects.joined_in AS joinedIn, users_projects.left_in AS leftIn, r1.description_ptbr AS role1, r1.icon AS role1icon, r2.description_ptbr AS role2, r2.icon AS role2icon, r3.description_ptbr AS role3, r3.icon AS role3icon FROM users_projects LEFT JOIN users ON users_projects.id_user_fk = users.id LEFT JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN roles AS r1 ON users_projects.main_role_fk = r1.id LEFT JOIN roles AS r2 ON users_projects.second_role_fk = r2.id LEFT JOIN roles AS r3 ON users_projects.third_role_fk = r3.id WHERE users_projects.confirmed = ${userId} AND users_projects.id_project_fk IN (SELECT projects.id FROM users_projects LEFT JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN projects_types ON projects.type = projects_types.id LEFT JOIN users_projects_status ON users_projects.status = users_projects_status.id WHERE users_projects.id_user_fk = ${userId} AND users_projects.confirmed IN(0,1,2) AND projects.id IS NOT NULL GROUP BY users_projects.id) ORDER BY projects.name;
...
Project.getMembers = (projectUsername, result) => {
  sql.query(`SELECT users_projects.id_user_fk AS id, users_projects.id_project_fk AS projectId, users_projects.joined_in AS joinedIn, users_projects.left_in AS leftIn, users_projects.confirmed, users.name, users.lastname, users.username, users.picture AS picture, users.bio, role1.name_ptbr AS role1, role2.name_ptbr AS role2, role3.name_ptbr AS role3, projects.name AS projectName, projects.username AS projectUsername, users_projects_status.id AS statusId, users_projects_status.title_ptbr AS statusName, users_projects_status.icon AS statusIcon, users_projects.admin, users_projects.active, users_projects.leader, users_projects.touring FROM users_projects LEFT JOIN projects ON users_projects.id_project_fk = projects.id LEFT JOIN users ON users_projects.id_user_fk = users.id LEFT JOIN roles AS role1 ON users_projects.main_role_fk = role1.id LEFT JOIN roles AS role2 ON users_projects.second_role_fk = role2.id LEFT JOIN roles AS role3 ON users_projects.third_role_fk = role3.id LEFT JOIN users_projects_status ON users_projects.status = users_projects_status.id WHERE projects.username = '${projectUsername}' AND users_projects.status IN(1,2,3,4,5) AND users.name IS NOT NULL ORDER BY users_projects.admin DESC, users_projects.leader DESC, users.name ASC`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
