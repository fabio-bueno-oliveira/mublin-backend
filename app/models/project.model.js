...
Project.findAllByUser = (userId, result) => {
  ...
  LEFT JOIN (SELECT events_invitations.id, events_invitations.response, events_invitations.id_event_fk, events_invitations.created, users.name, users.picture, users.username FROM events_invitations LEFT JOIN users ON events_invitations.id_user_who_invited_fk = users.id) AS events_invitations ON events.id = events_invitations.id_event_fk 
