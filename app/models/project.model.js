...
Project.findAllByUser = (userId, type, result) => {
  ...
  LEFT JOIN (SELECT id_project, DATE_FORMAT(project_notes.created,'%d/%m/%Y') AS note_date, note, users.name, users.picture, users.username FROM project_notes LEFT JOIN users ON project_notes.id_author = users.id ORDER BY project_notes.created DESC LIMIT 1) AS project_notes ON projects.id = project_notes.id_project 
