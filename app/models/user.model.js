...
User.addUsersProject = (loggedID, userId, projectId, status, main_role_fk, joined_in, left_in, active, leader, confirmed, admin, result) => {
  let x = jwt.verify(loggedID.slice(7), process.env.JWT_SECRET)
  if (x.result.id == userId) {
    sql.query(`INSERT INTO users_projects (id_user_fk, id_project_fk, active, status, main_role_fk, joined_in, left_in, leader, confirmed, admin) VALUES (${userId}, ${projectId}, ${active}, ${status}, ${main_role_fk}, ${joined_in}, ${left_in}, ${leader}, ${confirmed}, ${admin})`, (err, res) => {
        if (err) {
          //console.log("error: ", err);
          result(null, err);
          return;
...
User.findNotesByLoggedUserId = (loggedID, userId, result) => {
  let x = jwt.verify(loggedID.slice(7), process.env.JWT_SECRET)
  if (x.result.id == userId) {
    sql.query(`SELECT notes.id AS noteId, DATE_FORMAT(notes.created,'%d/%m/%Y às %H:%i:%s') AS noteCreated, UNIX_TIMESTAMP(notes.created) AS created, notes.id_user_owner AS ownerId, notes.id_project AS projectId, notes.title AS noteTitle, notes.description AS noteDescription, DATE_FORMAT(notes.target_date,'%d/%m/%Y às %H:%i:%s') AS noteTargetDate, CONCAT (users.name,' ',users.lastname) AS ownerName, users.picture AS ownerPicture, users.username AS ownerUsername, projects.name AS projectName, projects.username AS projectUsername, CONCAT('https://ik.imagekit.io/mublin/projects/tr:h-200,w-200,c-maintain_ratio/',notes.id_project,'/',projects.picture) AS projectPicture FROM notes LEFT JOIN users ON users.id = notes.id_user_owner LEFT JOIN projects ON projects.id = notes.id_project WHERE notes.id_user_owner = ${userId} OR notes.id IN (SELECT id_note FROM note_association WHERE id_user_associated = ${userId}) ORDER BY notes.created DESC`, (err, res) => {
      if (err) {
        result(err, null);
        return;
      }
...
User.checkProjectAdmin = (loggedID, projectUsername, result) => {
  let x = jwt.verify(loggedID.slice(7), process.env.JWT_SECRET)
  let errorMsg = {project: projectUsername, accessible: 1}
  sql.query(`SELECT users_projects.confirmed FROM users_projects WHERE users_projects.id_project_fk = (SELECT projects.id FROM projects WHERE projects.username = '${projectUsername}') AND users_projects.id_user_fk = ${x.result.id} AND users_projects.confirmed = 1 LIMIT 1`, (err, res) => {
    if (err) {
      result(err, null);
      return;
    }
