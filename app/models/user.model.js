...
User.findAllByKeyword = (keyword, result) => {
  ...
  //   WHERE title LIKE '%${keyword}%' OR description LIKE '%${keyword}%' HAVING showable_on_site = 1 AND date_opening >= CURDATE() ORDER BY title ASC LIMIT 50`, (err, res) => {
    sql.query(`
    SELECT users.id, users.name AS title, users.lastname AS extra1, users.username AS extra2, users.status AS extra3, IF(users.payment_plan = 2,'PRO','') AS price, IF(troles.rolename IS NOT NULL,troles.rolename,'Usuário') AS description, IF(users.picture IS NOT NULL,CONCAT('https://ik.imagekit.io/mublin/users/avatars/tr:h-200,w-200,c-maintain_ratio/',users.picture),'') AS image, 'Usuário' as category FROM users LEFT JOIN 
    (SELECT users_roles.id_user_fk,
            users_roles.id_role_fk,
            users_roles.main_activity, roles.id AS rid, roles.name_ptbr AS rolename, roles.description_ptbr AS roledesc 
            FROM users_roles LEFT JOIN roles ON users_roles.id_role_fk = roles.id 
...
User.updateGearSetup = (loggedID, setupId, name, description, result) => {
  ...
  sql.query(`UPDATE users_gear_setup SET name = '${name}', description = '${description}', updatedAt = '${dateTime}' WHERE id = ${setupId} AND id_user = ${x.result.id}`, (err, res) => {
      ...
      result(null, { success: true, message: 'Setup updated successfully' });
    }
  );
};

User.updateGearSetupImage = (loggedID, setupId, image, result) => {
  let x = jwt.verify(loggedID.slice(7), process.env.JWT_SECRET)
  sql.query(`UPDATE users_gear_setup SET image = '${image}', updatedAt = '${dateTime}' WHERE id = ${setupId} AND id_user = ${x.result.id}`, (err, res) => {
      if (err) {
        result(null, err);
        return;
      }
      ...
      result(null, { success: true, message: 'Setup image updated successfully' });
    }
  );
};
